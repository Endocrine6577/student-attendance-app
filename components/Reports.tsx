    ...
    return attendanceRecords.filter(record => {
      ...
      if (filterDate) {
        try {
            ...
            // Invalid date format in record, filter it out
            return false;
        }
      }
                ...
                filteredRecords.map((record, index) => {
                  ...
                  return (
                    <tr key={index} className="hover:bg-gray-700/30 print:hover:bg-transparent">
                      <td className="px-6 py-4 whitespace-nowrap">{isDateValid ? dateObj.toLocaleDateString() : 'Invalid Date'}</td>
                      <td className="px-6 py-4 whitespace-nowrap">{isDateValid ? dateObj.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : '-'}</td>
